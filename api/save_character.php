...
try {
    // Verifica duplicata pelo nome + imagem (únicos por personagem)
    $check = $db->prepare("SELECT id FROM favorites WHERE name = ? AND image = ? AND user_id = ?");
    $check->execute([$name, $image, $user_id]);
    if ($check->fetch()) {
